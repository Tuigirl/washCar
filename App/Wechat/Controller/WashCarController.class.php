...
class WashCarController extends CommonController
{
    ...
    {
        $this->rocord = D('Wechat/WashRecord');
    ...
    public function toPay()
    {
        ...
        //获取所有的设备
        $device = $this->washcar->washDevice();
        if (in_array($data['DeviceSerial'], $device)) {
            //检测洗车是否可工作
            $res = $this->washcar->checkDevice($data['DeviceSerial']);
            if ($res['status']) {
                ajax_output(1, $res['message']);
    ...
    public function getOrderStatus()
    {
        ...
        $orderInfo = $this->rocord->where($where)->order('pay_time DESC')->find();
