...
export default {
  ...
  computed: {
    ...mapState(['openid', 'city_list', 'CarUsedData']),
  },

  mounted() {
    if (!this.openid && this.$route.query.openid) {
  ...
  methods: {
    form_submit() {//表单提交事件
      ...
      if (!RegVerify('name', submit_data.name)) {
        _.$vux.toast.show({
          text: '姓名限定为中英文',
          type: 'text',
          isShowMask: true,
      ...
      if (!RegVerify('tel', submit_data.mobile)) {
        _.$vux.toast.show({
          text: '请输入正确的手机号',
          type: 'text',
          isShowMask: true,
      ...
      submit_data.openid = this.openid
      submit_data.sign = _.GetSign(submit_data)//追加签名验证

      _.$http.post('/index.php/Wechat/SecondHand/sellingCar', submit_data)
        .then((response) => {// 响应成功回调
          if (response.code === 0) {//提交成功
            this.$router.replace({
              name: 'CarUsed__Result',
              params: {car_info: response.data}
            })
          }
        })
    },
