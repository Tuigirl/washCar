<template>
  <div>
    ...
    >
      <scroller ref="scrollerTwo" class="scrollerTwo" :style="{top: $store.state.isAndroid?'46px':'0px'}" :height="$store.state.isAndroid?'-46':''" lock-x scrollbar-y v-show="isShowTwo">
        <div class="page__list">
          <group gutter="0">
            <cell
...
export default {
  ...
  mounted() {
    ...
    _.$http.post('/index.php/Xiaochengxu/SecondHand/getCarModelsList1').then((response) => {// 响应成功回调
      ...
      if (response.code === 0) {
        let data = {}, temp = response.data[0]['car']
  ...
  methods: {
    ...
    getBrandListTwo(brand_id) {
      ...
      ,{
        ...
        if (response.code === 0) {
          _.brand_list_2 = response.data//更新列表
