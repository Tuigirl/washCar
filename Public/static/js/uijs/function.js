...
//执行回传函数
function ExePostBack3(objId, objmsg) {
    if ($(".checkall input:checked").size() < 1) {
        $.ligerDialog.warn("对不起，请选中您要操作的记录！");
        return false;
    }
    else if ($(".checkall input:checked").size() > 1) {
        $.ligerDialog.warn("对不起，只能选择一条记录！");
        return false;
    }
    else {
        return true;
    ...
    //        msg = objmsg;
    //    }
//    $.ligerDialog.confirm("", "提示信息", function (result) {
//        if (result) {
//            __doPostBack(objId, '');
//        }
//    });
...
function Upload(action, repath, uppath, iswater, isthumbnail, filepath) {
    ...
    if(arguments.length == 4){
        sendUrl = "../tolls/upload_ajax.ashx?action=" + action + "&ReFilePath=" + repath + "&UpFilePath=" + uppath + "&IsWater=" + iswater;
    }
    //判断是否生成宿略图
    ...
    if (arguments.length == 6) {
        sendUrl = filepath + "upload_ajax.ashx?action=" + action + "&ReFilePath=" + repath + "&UpFilePath=" + uppath + "&IsWater=" + iswater + "&IsThumbnail=" + isthumbnail;
    }
    //开始提交
    $("#form1").ajaxSubmit({
