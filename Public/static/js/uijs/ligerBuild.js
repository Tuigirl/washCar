					...
					var g = {
						onResize : function() {
							...
							if (g.accordion.parent()[0].tagName.toLowerCase() == "body") {
								...
								windowHeight -= parseInt(g.layout.parent().css(
										'paddingTop'));
								windowHeight -= parseInt(g.layout.parent().css(
										'paddingBottom'));
						...
						setHeight : function(height) {
							...
							$("> .l-accordion-content", g.accordion).height(
									height);
						}
					};
					g.accordion = $(this);
...
var ligerDialogImagePath = "../../scripts/ui/skins/Aqua/image/caryu/dialog/";
(function($) {
	...
	$.ligerDialog.open = function(p) {
		p = $.extend({}, $.ligerDefaults.Dialog, $.ligerDefaults.DialogString,
				p || {});
		var g = {
					...
					g.dialog.ligerResizable({
						onStopResize : function(current, e) {
							...
							if (current.diffTop != undefined) {
								...
								$(".l-dialog-content", g.dialog.body).height(
										current.newHeight
												- 46
												- $(".l-dialog-buttons",
														g.dialog).height());
		...
		if (p.type)
			...
			$(".l-dialog-content", g.dialog.body).addClass(
			...
			$(p.buttons)
					.each(
							function(i, item) {
								...
								$(".l-dialog-buttons-inner", g.dialog.body)
										.prepend(btn);
		...
		$(".l-dialog-buttons-inner", g.dialog).append(
				"<div class='l-clear'></div>");
		p.width && g.dialog.body.width(p.width - 26);
		if (p.height) {
			$(".l-dialog-content", g.dialog.body).height(
					p.height - 46 - $(".l-dialog-buttons", g.dialog).height());
		}
		...
		if ($.browser.msie && (ie55 || ie6)) {
			$(".l-dialog-tl:first", g.dialog)
					.css(
							{
								"background" : "none",
								"filter" : "progid:DXImageTransform.Microsoft.AlphaImageLoader(src='"
										+ ligerDialogImagePath
										+ "dialog-tl.png',sizingMethod='crop');"
							});
			$(".l-dialog-tc:first", g.dialog)
					.css(
							{
								"background" : "none",
								"filter" : "progid:DXImageTransform.Microsoft.AlphaImageLoader(src='"
										+ ligerDialogImagePath
										+ "ie6/dialog-tc.png',sizingMethod='crop');"
							});
			$(".l-dialog-tr:first", g.dialog)
					.css(
							{
								"background" : "none",
								"filter" : "progid:DXImageTransform.Microsoft.AlphaImageLoader(src='"
										+ ligerDialogImagePath
										+ "dialog-tr.png',sizingMethod='crop');"
							});
			$(".l-dialog-cl:first", g.dialog)
					.css(
							{
								"background" : "none",
								"filter" : "progid:DXImageTransform.Microsoft.AlphaImageLoader(src='"
										+ ligerDialogImagePath
										+ "ie6/dialog-cl.png',sizingMethod='crop');"
							});
			$(".l-dialog-cr:first", g.dialog)
					.css(
							{
								"background" : "none",
								"filter" : "progid:DXImageTransform.Microsoft.AlphaImageLoader(src='"
										+ ligerDialogImagePath
										+ "ie6/dialog-cr.png',sizingMethod='crop');"
							});
			$(".l-dialog-bl:first", g.dialog)
					.css(
							{
								"background" : "none",
								"filter" : "progid:DXImageTransform.Microsoft.AlphaImageLoader(src='"
										+ ligerDialogImagePath
										+ "dialog-bl.png',sizingMethod='crop');"
							});
			$(".l-dialog-bc:first", g.dialog)
					.css(
							{
								"background" : "none",
								"filter" : "progid:DXImageTransform.Microsoft.AlphaImageLoader(src='"
										+ ligerDialogImagePath
										+ "ie6/dialog-bc.png',sizingMethod='crop');"
							});
			$(".l-dialog-br:first", g.dialog)
					.css(
							{
								"background" : "none",
								"filter" : "progid:DXImageTransform.Microsoft.AlphaImageLoader(src='"
										+ ligerDialogImagePath
										+ "dialog-br.png',sizingMethod='crop');"
							});
		}
		var left = 0;
		var top = 0;
		...
		if (p.top != null)
			...
			top = 0.5 * ($(window).height() - g.dialog.height())
					+ $(window).scrollTop() - 10;
		g.dialog.css({
			left : left,
			top : top
				...
				.each(function() {
					...
					var g = {
						...
						stop : function(e) {
							...
							$("body").css("cursor", "");
							if (p.onStopDrag)
								p.onStopDrag(g.current, e);
							g.current = null;
						},
						applyDrag : function() {
							if (g.current.diffX) {
								g.target.css("left",
										(g.current.left + g.current.diffX));
							}
							if (g.current.diffY) {
								g.target.css("top",
										(g.current.top + g.current.diffY));
							}
						}
					};
					g.target = $(this);
					if (p.handler == undefined || p.handler == null)
						...
						g.handler = (typeof p.handler == 'string' ? $(
								p.handler, this) : p.handle);
...
if (typeof (LigerUIMenu) == "undefined")
	LigerUIMenu = {};
(function($) {
	$.ligerDefaults = $.ligerDefaults || {};
	$.ligerDefaults.Menu = {
	...
	$.ligerMenu = function(p) {
		...
		var g = {
			...
			isEnable : function(itemid) {
				return !$("> .l-menu-item[menuitemid=" + itemid + "]",
						g.menu.items).hasClass("l-menu-item-disable");
			},
			...
			addItem : function(item, menu) {
				...
				if (item.children) {
					if (ditem.attr("menuitemid") == undefined)
						ditem.attr("menuitemid", new Date().getTime());
			...
			createMenu : function(parentMenuItemID) {
				...
				parentMenuItemID
						&& menu.attr("parentmenuitemid", parentMenuItemID);
		...
		this
				.each(function() {
					...
					if ($("> div[position=bottom]", g.layout).length > 0) {
						g.bottom = $("> div[position=bottom]", g.layout).wrap(
								'<div class="l-layout-bottom"></div>').parent();
					...
					if ($("> div[position=left]", g.layout).length > 0) {
						...
						if (lefttitle) {
							...
							$(".l-layout-header-inner", g.left.header).html(
									lefttitle);
						}
					...
					if ($("> div[position=center]", g.layout).length > 0) {
						g.center = $("> div[position=center]", g.layout).wrap(
					...
					if ($("> div[position=right]", g.layout).length > 0) {
						g.right = $("> div[position=right]", g.layout).wrap(
								'<div class="l-layout-right"></div>').parent();
						...
						if (righttitle) {
							...
							$(".l-layout-header-inner", g.right.header).html(
									righttitle);
						}
					...
					g.leftCollapse.toggle = $(
							"> .l-layout-collapse-left-toggle", g.leftCollapse);
					...
					var g = {
						init : function() {
							...
							for (var i = 0; i < handles.length; i++) {
								...
								case "nw":
									g.target
											.append('<div class="l-resizable-h-l" direction="nw"></div>');
									break;
								case "ne":
									g.target
											.append('<div class="l-resizable-h-r" direction="ne"></div>');
									break;
								case "n":
									g.target
											.append('<div class="l-resizable-h-c" direction="n"></div>');
									break;
								case "w":
									g.target
											.append('<div class="l-resizable-c-l" direction="w"></div>');
									break;
								case "e":
									g.target
											.append('<div class="l-resizable-c-r" direction="e"></div>');
									break;
								case "sw":
									g.target
											.append('<div class="l-resizable-f-l" direction="sw"></div>');
									break;
								case "se":
									g.target
											.append('<div class="l-resizable-f-r" direction="se"></div>');
									break;
								case "s":
									g.target
											.append('<div class="l-resizable-f-c" direction="s"></div>');
									break;
								}
							}
							$("> .l-resizable-h-c , > .l-resizable-f-c",
									g.target).width(g.target.width());
							$("> .l-resizable-c-l , > .l-resizable-c-r",
									g.target).height(g.target.height());
						...
						start : function(e, dir) {
							...
							$(
									"<div class='l-window-mask-nobackground' style='display: block;'></div>")
									.appendTo($("body"));
						...
						drag : function(e) {
							...
							var moveWidth = (e.pageX || e.screenX)
									- g.current.left;
							var moveHeight = (e.pageY || e.clientY)
									- g.current.top;
							if (dir.indexOf("e") >= 0)
								moveWidth -= width;
							if (dir.indexOf("s") >= 0)
								moveHeight -= height;
							if (dir != "n" && dir != "s") {
								width += (dir.indexOf("w") >= 0) ? -moveWidth
										: moveWidth;
							}
							if (width >= p.minWidth) {
								if (dir.indexOf("w") >= 0) {
							...
							if (dir != "w" && dir != "e") {
								height += (dir.indexOf("n") >= 0) ? -moveHeight
										: moveHeight;
							}
							...
							if (isNaN(g.current.diffTop))
								g.current.diffTop = 0;
							g.current.diffLeft = parseInt(resizableObj.style.left);
							if (isNaN(g.current.diffLeft))
								g.current.diffLeft = 0;
							$("body").css("cursor", dir + '-resize');
							if (p.onResize)
								p.onResize(g.current, e);
						},
						stop : function(e) {
							if ($.browser.msie || $.browser.safari)
								$('body').unbind('selectstart');
							...
							if (!p.onStopResize)
								g.applyResize();
							else if (p.onStopResize(g.current, e) != false)
								g.applyResize();
						...
						applyResize : function() {
							...
							if (g.current.diffTop != undefined) {
								if (g.current.diffTop != 0)
									g.target.css({
										top : top + g.current.diffTop
									});
								if (g.current.diffLeft != 0)
									g.target.css({
										left : left + g.current.diffLeft
									});
								g.target.css({
		...
		this
				.each(function() {
					...
					if (tipid) {
						...
						tip = $('<div class="l-verify-tip"><div class="l-verify-tip-corner"></div><div class="l-verify-tip-content">'
								+ p.content + '</div></div>');
					...
					tip.css(
							{
								left : $(this).position().left
										+ $(this).width() + p.distanceX,
								top : $(this).position().top + p.distanceY
							}).show();
					$(this).attr("ligerTipId", tip.attr("id"));
					p.width
							&& $("> .l-verify-tip-content", tip).width(
									p.width - 8);
					p.height
							&& $("> .l-verify-tip-content", tip)
									.width(p.height);
					p.appendIdTo
							&& p.appendIdTo.attr("ligerTipId", tip.attr("id"));
				...
				.each(function() {
					...
					var g = {
						setTabButton : function() {
							...
							if (sumwidth > mainwidth) {
								...
								if ($(g.tab).attr("toolsid")) {
									$(".l-tab-links-right", g.tab.links).css(
											"right", "17px");
								}
								g.setTabButtonEven();
								return true;
						...
						moveToPrevTabItem : function() {
							var btnWitdth = $(".l-tab-links-left", g.tab.links)
									.width();
							var leftList = new Array();
							$("li", g.tab.links.ul)
									.each(
											function(i, item) {
												var currentItemLeft = -1
														* btnWitdth;
												if (i > 0) {
							...
							var currentLeft = -1
									* parseInt(g.tab.links.ul.css("left"));
							for (var i = 0; i < leftList.length - 1; i++) {
								if (leftList[i] < currentLeft
										&& leftList[i + 1] >= currentLeft) {
									g.tab.links.ul.animate({
						...
						moveToNextTabItem : function() {
							var btnWitdth = $(".l-tab-links-right", g.tab)
									.width()
									+ $(".l-tab-links-tools", g.tab.links)
											.width();
							var sumwidth = 0;
							...
							for (var i = tabItems.length - 1; i >= 0; i--) {
								var currentItemLeft = sumwidth - mainwidth
										+ btnWitdth + p.marginleft;
								if (i != tabItems.length - 1) {
									currentItemLeft = parseInt(leftList[tabItems.length
											- 2 - i])
											- ($(tabItems[i + 1]).width() + p.marginleft);
								}
								leftList.push(currentItemLeft);
							}
							var currentLeft = -1
						...
						overrideTabItem : function(targettabid, options) {
							if (p.onBeforeOverrideTabItem
									&& p.onBeforeOverrideTabItem(targettabid) == false)
								return false;
							...
							p.onAfterOverrideTabItem
									&& p.onAfterOverrideTabItem(targettabid);
						},
						selectTabItem : function(tabid) {
							if (p.onBeforeSelectTabItem
									&& p.onBeforeSelectTabItem(tabid) == false)
								return false;
							...
							p.onAfterSelectTabItem
									&& p.onAfterSelectTabItem(tabid);
						},
						moveToCurrTabItem : function(tabid) {
							...
							$("li", g.tab.links.ul).each(
									function() {
										selecttabrightwidth += $(this).width()
												+ p.marginleft;
										if ($(this).attr("tabid") == tabid) {
											return false;
										}
										selecttableftwidth += $(this).width()
												+ p.marginleft;
									});
							var tableftnum = parseInt($(g.tab.links.ul).css(
									"left"));
							if (selecttableftwidth < (0 - tableftnum)) {
								g.tab.links.ul.animate({
									left : -1
											* (selecttableftwidth - btnWitdth)
								});
							} else if ((selecttabrightwidth + tableftnum) > mainwidth) {
								btnWitdth += $(".l-tab-links-tools",
										g.tab.links).width();
								g.tab.links.ul
										.animate({
											left : (-1 * (selecttabrightwidth
													- mainwidth + btnWitdth + p.marginleft))
										});
							}
						},
						...
						addTabItem : function(options) {
							...
							if (p.onBeforeAddTabItem
									&& p.onBeforeAddTabItem(tabid) == false)
								return false;
							var tabid = options.tabid;
							if (tabid == undefined)
								tabid = g.getNewTabid();
						...
						removeTabItem : function(tabid) {
							if (p.onBeforeRemoveTabItem
									&& p.onBeforeRemoveTabItem(tabid) == false)
								return false;
							...
							$(".l-tab-content-item[tabid=" + tabid + "]",
									g.tab.content).remove();
							$("li[tabid=" + tabid + "]", g.tab.links.ul)
									.remove();
							g.setTabButton();
							p.onAfterRemoveTabItem
									&& p.onAfterRemoveTabItem(tabid);
						},
					...
					if ($(g.tab).attr("toolsid")) {
						g.tab.links.append('<div id="'
								+ $(g.tab).attr("toolsid")
								+ '" class="l-tab-links-tools"></div>');
					}
					g.tab.links.ul = $("ul", g.tab.links);
					var haslselected = $("> div[lselected=true]", g.tab.content).length > 0;
					g.selectedTabId = $("> div[lselected=true]", g.tab.content)
							.attr("tabid");
					$("> div", g.tab.content)
							.each(
									function(i, box) {
				...
				.each(function() {
					...
					var g = {
						...
						getSelected : function() {
							...
							if (node.target) {
								var treedataindex = parseInt($(node.target)
										.attr("treedataindex"));
								node.data = po.getDataNodeByTreeDataIndex(
						...
						upgrade : function(treeNode) {
							...
							$("." + po.getChildNodeClassName(), treeNode)
									.each(
											function() {
												$(this)
														.removeClass(
																po
																		.getChildNodeClassName())
														.addClass(
																po
																		.getParentNodeClassName(true));
											});
						},
						demotion : function(treeNode) {
							...
							$("." + po.getParentNodeClassName(true), treeNode)
									.each(
											function() {
												$(this)
														.removeClass(
						...
						remove : function(treeNode) {
							var treedataindex = parseInt($(treeNode).attr(
									"treedataindex"));
							var treenodedata = po.getDataNodeByTreeDataIndex(
									g.data, treedataindex);
							if (treenodedata)
								po
										.setTreeDataStatus([ treenodedata ],
												'delete');
						...
						append : function(parentNode, newdata) {
							...
							if (!parentNode) {
								if ($("> li:last", g.tree).length > 0)
									po.setTreeItem($("> li:last", g.tree)[0], {
										isLast : false
									});
								var gridhtmlarr = po.getTreeHTMLByData(newdata,
										1, [], true);
							...
							if ($("> .l-children > li:last", treeitem).length > 0)
								po.setTreeItem($("> .l-children > li:last",
										treeitem)[0], {
									isLast : false
								});
							var isLast = [];
					...
					var po = {
						getDataNodeByTreeDataIndex : function(data,
								treedataindex) {
						...
						setTreeDataStatus : function(data, status) {
							$(data).each(
									function() {
										...
										if (this.children) {
											po.setTreeDataStatus(this.children,
													status);
										}
									});
						},
						...
						getTreeHTMLByData : function(data, outlineLevel,
								isLast, isExpand) {
							...
							for (var i = 0; i < data.length; i++) {
								...
								if (g.hasChildren(data[i])) {
									...
									treehtmlarr.push(po.getTreeHTMLByData(
											data[i].children, outlineLevel + 1,
						...
						applyTree : function() {
							...
							var gridhtmlarr = po.getTreeHTMLByData(g.data, 1,
									[], true);
							gridhtmlarr[gridhtmlarr.length - 1] = gridhtmlarr[0] = "";
									...
									.click(function(e) {
										...
										var treenodedata = po
												.getDataNodeByTreeDataIndex(
														g.data, treedataindex);
										var treeitembtn = $(
												".l-body:first .l-expandable-open:first,.l-body:first .l-expandable-close:first",
												treeitem);
										...
										if ($(obj).hasClass("l-checkbox")) {
											if (p.autoCheckboxEven) {
												...
												po
														.setParentCheckboxStatus(treeitem);
											}
										} else if (treeitembtn
						...
						setParentCheckboxStatus : function(treeitem) {
							...
							if (treeitem.parent().parent("li").length > 0)
								po.setParentCheckboxStatus(treeitem.parent()
										.parent("li"));
						},
						convertData : function(data) {
							if (!data || !data.length)
								return [];
							var isolate = function(pid) {
							...
							var currentIndex = 0;
							var getItem = function(container, id) {
								if (!container.length)
									return null;
								for (var i = 0; i < container.length; i++) {
									...
									if (container[i].children) {
										var finditem = getItem(
												container[i].children, id);
										if (finditem)
											return finditem;
									}
								}
							...
							while (insertedLength + isolateLength < itemLength) {
								...
								if (pid == p.topParentIDValue) {
									getItem(targetData, id) == null
											&& addItem(targetData, item);
								} else {
